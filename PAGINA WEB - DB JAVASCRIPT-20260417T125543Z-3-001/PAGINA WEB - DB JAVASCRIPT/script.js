...
// ==========================================================
// El carrito se carga con localStorage
...
const aplicarFiltrosYOrdenar = () => {
    let productosFiltrados = db.getAllProducts();
...
// ==========================================================
// 5. LÓGICA DE DETALLE DE PRODUCTO
// ==========================================================
...
db.getAllLocations().forEach(sede => { // <--- CAMBIO AQUÍ
...
// ==========================================================
// 7. LÓGICA DE CONTACTO
// ==========================================================

const validarCampoContacto = (campo) => {
...
const validarFormularioContacto = (form) => {
    let esValido = true;
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('EcoMarket - Interacción cargada.');
    ...
    if (formContacto) {
        ...
        // Formateo automático para el campo de teléfono
        const inputTelefono = formContacto.querySelector('#input-telefono');
    if (inputTelefono) {
        inputTelefono.addEventListener('input', function(e) {
            // 1. Elimina todo lo que no sea un dígito
            let valor = e.target.value.replace(/\D/g, '');
            
            // 2. Limita el valor a un máximo de 9 dígitos
            valor = valor.substring(0, 9);
            
            // 3. Aplica el formato "XXX XXX XXX"
            let formateado = '';
            if (valor.length > 6) {
                formateado = `${valor.substring(0, 3)} ${valor.substring(3, 6)} ${valor.substring(6, 9)}`;
            ...
            // 4. Actualiza el valor del campo
            e.target.value = formateado;
        });
    }
        ...
        formContacto.addEventListener('submit', function(event) {
            ...
            if (validarFormularioContacto(formContacto)) {
                ...
                if (mensajeExito) {
                    mensajeExito.classList.remove('d-none');
                    formContacto.reset();
                    formContacto.querySelectorAll('.is-valid, .is-invalid').forEach(input => input.classList.remove('is-valid', 'is-invalid'));
        ...
        const campos = formContacto.querySelectorAll('input, select, textarea');
